...
export interface UiSchemaEditorDialogProps extends InstanceProps<ICardSchemaMetaWithIds | null> {
  uiSchema: ICardSchemaMetaWithIds<UnknownRecord>
}
  ...
  const validateAndParseJson = (json: string) => {
    try {
      ...
      if (!parsed.layout || !Array.isArray(parsed.layout)) {
        throw new Error('UI Schema must have a layout array')
      }

      setError('')
      setIsValid(true)
      return parsed as ICardSchemaMetaWithIds<UnknownRecord>
    } catch (err) {
