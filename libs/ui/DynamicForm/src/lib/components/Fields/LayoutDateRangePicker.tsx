...
import { IInputDateRangeLayoutField, IRestLayoutFields } from '../../types/types'
...
const LayoutDateRangePicker: React.FC<IInputDateRangeLayoutField> = (field) => {
  const {
    path,
    label,
    startDateLabel,
  ...
  const handleStartDateChange = (newStartDate: Date | null, onChange: (value: any) => void) => {
  ...
  const handleEndDateChange = (newEndDate: Date | null, onChange: (value: any) => void) => {
    const currentRange = currentValue || { startDate: null, endDate: null }
              ...
              <DatePicker
                ...
                slotProps={{
                  textField: {
                    ...
                    fullWidth: true,
                    error: !!error,
                    helperText: (error as any)?.[endDatePath]?.message,
                    variant: 'outlined',
                    sx: {
                      flex: 1,
                      '& .MuiInputBase-root': {
