...
import { ICardSchemaMetaWithIds, ICollection } from '@pro3/DynamicForm'
import EditIcon from '@mui/icons-material/Edit'
import { openUiSchemaEditorDialog } from '@pro3/DynamicForm'
import { UnknownRecord } from '@pro3/DynamicForm'
...
const Header = ({
  ...
  setUiSchema: (uiSchema: ICardSchemaMetaWithIds<UnknownRecord>) => void
  isLoading: boolean
  ...
  const handleOpenJsonEditor = async () => {
    try {
      const result = await openUiSchemaEditorDialog({
        uiSchema: uiSchema as ICardSchemaMetaWithIds<UnknownRecord>
      })
      if (result) {
        setUiSchema(result)
      }
